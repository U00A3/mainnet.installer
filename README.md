...
**Last mirror update:** 2026-07-10 (UTC)
...
| Field | Value |
|--------|--------|
| **File name** | `rbn-installer-mainnet-v1.3.16.run` |
| **Installer version (UI)** | see Vine portal |
| **Binary version** | v1.3.16 |
...
The installer file is **`rbn-installer-mainnet-v1.3.16.run`** in that folder.

### Option B - one command: download and verify (`wget`)

It saves **`rbn-installer-mainnet-v1.3.16.run`** in the current folder, then checks the SHA-256 from this README. **Only** the `.run` file is downloaded, not the README or `.sha256` sidecar.

```bash
wget -O 'rbn-installer-mainnet-v1.3.16.run' --show-progress \
  'https://raw.githubusercontent.com/U00A3/mainnet.installer/main/rbn-installer-mainnet-v1.3.16.run' \
  && echo "c191a1383b14fbd5d2c307e5b3d2b59a4ef05a0a74eabe00fd3546db01be1157  rbn-installer-mainnet-v1.3.16.run" | sha256sum -c
```

If you see **`rbn-installer-mainnet-v1.3.16.run: OK`**, the file matches the SHA-256.

If verification **fails**, delete the file (`rm -f 'rbn-installer-mainnet-v1.3.16.run'`) before you try again.
...
**Expected SHA-256 for `rbn-installer-mainnet-v1.3.16.run`:**

```
c191a1383b14fbd5d2c307e5b3d2b59a4ef05a0a74eabe00fd3546db01be1157
```

On **Linux**, compute the hash of the file you have (from the directory where `rbn-installer-mainnet-v1.3.16.run` is):

```bash
sha256sum rbn-installer-mainnet-v1.3.16.run
```

**What you should see** (one line; the first field must match the box above and the official sources):

```
c191a1383b14fbd5d2c307e5b3d2b59a4ef05a0a74eabe00fd3546db01be1157  rbn-installer-mainnet-v1.3.16.run
```
...
**`rbn-installer-mainnet-v1.3.16.run: OK`** means the file matches this README’s hash. If the command reports a failure, **do not run** the installer; remove the file and obtain a fresh copy, then compare again with Vine / Discord / other official operator comms.
