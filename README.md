...
**Last mirror update:** 2026-09-14 (UTC)
...
| Field | Value |
|--------|--------|
| **File name** | `rbn-installer-mainnet-v1.3.26.run` |
| **Installer version (UI)** | see Vine portal |
| **Binary version** | v1.3.26 |
...
The installer file is **`rbn-installer-mainnet-v1.3.26.run`** in that folder.

### Option B - one command: download and verify (`wget`)

It saves **`rbn-installer-mainnet-v1.3.26.run`** in the current folder, then checks the SHA-256 from this README. **Only** the `.run` file is downloaded, not the README or `.sha256` sidecar.

```bash
wget -O 'rbn-installer-mainnet-v1.3.26.run' --show-progress \
  'https://raw.githubusercontent.com/U00A3/mainnet.installer/main/rbn-installer-mainnet-v1.3.26.run' \
  && echo "3d48021470dd0c9f1ce550fb2611b4347f61cef12c3ae695d636893c18ce992f  rbn-installer-mainnet-v1.3.26.run" | sha256sum -c
```

If you see **`rbn-installer-mainnet-v1.3.26.run: OK`**, the file matches the SHA-256.

If verification **fails**, delete the file (`rm -f 'rbn-installer-mainnet-v1.3.26.run'`) before you try again.
...
**Expected SHA-256 for `rbn-installer-mainnet-v1.3.26.run`:**

```
3d48021470dd0c9f1ce550fb2611b4347f61cef12c3ae695d636893c18ce992f
```

On **Linux**, compute the hash of the file you have (from the directory where `rbn-installer-mainnet-v1.3.26.run` is):

```bash
sha256sum rbn-installer-mainnet-v1.3.26.run
```

**What you should see** (one line; the first field must match the box above and the official sources):

```
3d48021470dd0c9f1ce550fb2611b4347f61cef12c3ae695d636893c18ce992f  rbn-installer-mainnet-v1.3.26.run
```
...
**`rbn-installer-mainnet-v1.3.26.run: OK`** means the file matches this README’s hash. If the command reports a failure, **do not run** the installer; remove the file and obtain a fresh copy, then compare again with Vine / Discord / other official operator comms.
