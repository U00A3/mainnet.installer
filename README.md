...
**Last mirror update:** 2026-04-04 (UTC)
...
| Field | Value |
|--------|--------|
| **File name** | `rbn-installer-mainnet-v1.3.5.run` |
| **Installer version (UI)** | see Vine portal |
| **Binary version** | v1.3.5 |
...
The installer file is **`rbn-installer-mainnet-v1.3.5.run`** in that folder.

### Option B - one command: download and verify (`wget`)

It saves **`rbn-installer-mainnet-v1.3.5.run`** in the current folder, then checks the SHA-256 from this README. **Only** the `.run` file is downloaded, not the README or `.sha256` sidecar.

```bash
wget -O 'rbn-installer-mainnet-v1.3.5.run' --show-progress \
  'https://raw.githubusercontent.com/U00A3/mainnet.installer/main/rbn-installer-mainnet-v1.3.5.run' \
  && echo "1bcea7b1fe1aa25670d2976428ad6af14ece8fce56e7ed195dc414ad28592257  rbn-installer-mainnet-v1.3.5.run" | sha256sum -c
```

If you see **`rbn-installer-mainnet-v1.3.5.run: OK`**, the file matches the SHA-256.

If verification **fails**, delete the file (`rm -f 'rbn-installer-mainnet-v1.3.5.run'`) before you try again.
...
**Expected SHA-256 for `rbn-installer-mainnet-v1.3.5.run`:**

```
1bcea7b1fe1aa25670d2976428ad6af14ece8fce56e7ed195dc414ad28592257
```

On **Linux**, compute the hash of the file you have (from the directory where `rbn-installer-mainnet-v1.3.5.run` is):

```bash
sha256sum rbn-installer-mainnet-v1.3.5.run
```

**What you should see** (one line; the first field must match the box above and the official sources):

```
1bcea7b1fe1aa25670d2976428ad6af14ece8fce56e7ed195dc414ad28592257  rbn-installer-mainnet-v1.3.5.run
```
...
**`rbn-installer-mainnet-v1.3.5.run: OK`** means the file matches this README’s hash. If the command reports a failure, **do not run** the installer; remove the file and obtain a fresh copy, then compare again with Vine / Discord / other official operator comms.
