...
**Last mirror update:** 2026-08-11 (UTC)
...
| Field | Value |
|--------|--------|
| **File name** | `rbn-installer-mainnet-v1.3.20.run` |
| **Installer version (UI)** | see Vine portal |
| **Binary version** | v1.3.20 |
...
The installer file is **`rbn-installer-mainnet-v1.3.20.run`** in that folder.

### Option B - one command: download and verify (`wget`)

It saves **`rbn-installer-mainnet-v1.3.20.run`** in the current folder, then checks the SHA-256 from this README. **Only** the `.run` file is downloaded, not the README or `.sha256` sidecar.

```bash
wget -O 'rbn-installer-mainnet-v1.3.20.run' --show-progress \
  'https://raw.githubusercontent.com/U00A3/mainnet.installer/main/rbn-installer-mainnet-v1.3.20.run' \
  && echo "e0fd865846e7c3672d7210c8f63e26ff5d122580b52a63d544a19dad4c7b73ad  rbn-installer-mainnet-v1.3.20.run" | sha256sum -c
```

If you see **`rbn-installer-mainnet-v1.3.20.run: OK`**, the file matches the SHA-256.

If verification **fails**, delete the file (`rm -f 'rbn-installer-mainnet-v1.3.20.run'`) before you try again.
...
**Expected SHA-256 for `rbn-installer-mainnet-v1.3.20.run`:**

```
e0fd865846e7c3672d7210c8f63e26ff5d122580b52a63d544a19dad4c7b73ad
```

On **Linux**, compute the hash of the file you have (from the directory where `rbn-installer-mainnet-v1.3.20.run` is):

```bash
sha256sum rbn-installer-mainnet-v1.3.20.run
```

**What you should see** (one line; the first field must match the box above and the official sources):

```
e0fd865846e7c3672d7210c8f63e26ff5d122580b52a63d544a19dad4c7b73ad  rbn-installer-mainnet-v1.3.20.run
```
...
**`rbn-installer-mainnet-v1.3.20.run: OK`** means the file matches this README’s hash. If the command reports a failure, **do not run** the installer; remove the file and obtain a fresh copy, then compare again with Vine / Discord / other official operator comms.
